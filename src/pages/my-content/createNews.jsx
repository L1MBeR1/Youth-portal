...
function CreateNews() {
	...
	const handleSubmit = async e => {
		...
		try {
			...
			console.error(error);
			setErrorMessage(error.message || 'Произошла ошибка при создании новости');
		} finally {
			setIsLoading(false);
		}
	};
		...
		<Box>
			<Stack direction={'column'} spacing={3}>
				<Stack direction={'column'} spacing={2}>
					...
					<Typography level='title-xl'>Создание новости</Typography>
				</Stack>
				<Typography level='body-md'>
					Заполните форму ниже, чтобы опубликовать новую новость. Необходимо
					заполнить все поля.
				</Typography>
