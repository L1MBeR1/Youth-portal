...
import { FormatAlignLeft, Image, Fullscreen } from '@mui/icons-material';  // Иконки из MUI
...
function DraggableCard({ title, description, type, icon }) {
    ...
    return (
        <Card
            ref={drag}
            variant="outlined"
            sx={{ p: 2, opacity: isDragging ? 0.5 : 1, cursor: 'move', display: 'flex', alignItems: 'center', gap: 2, borderRadius: 2, boxShadow: 'md' }}
        >
            <Box sx={{ minWidth: 40, minHeight: 40, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {icon}
            </Box>
            <Box>
...
function ToolsSidebar() {
    return (
        <Card variant="outlined" sx={{ p: 3, bgcolor: 'background.level1', boxShadow: 'md', borderRadius: 2, width: { xs: '100%', sm: 320 } }}>
            <Typography level="h2" sx={{ mb: 3, fontSize: '1.5rem', fontWeight: 'bold' }}>
                Container Templates
            </Typography>
