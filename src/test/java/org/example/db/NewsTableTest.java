...
public class NewsTableTest {
    ...
    public void deleteData() throws SQLException {
        try (Statement stmt = connection.createStatement()) {
            // Удаление данных из таблицы news и user_login_data
            stmt.executeUpdate("DELETE FROM public.news WHERE author_id = 1;");
            stmt.executeUpdate("DELETE FROM public.user_login_data WHERE id = 1;");
        }
    }

    @Test
    public void testNewsTableExists() throws SQLException {
        try (Statement stmt = connection.createStatement()) {
            ...
            assertTrue(tableExists, "Table 'news' does not exist");
        }
    }

    @Test
    public void testNewsColumns() throws SQLException {
    ...
    @Test
    public void testNewsConstraints() throws SQLException {
        // Проверка первичного ключа
        try (Statement stmt = connection.createStatement()) {
            ResultSet rs = stmt.executeQuery(
                    "SELECT conname " +
                            "FROM pg_constraint " +
                            "WHERE conrelid = 'public.news'::regclass AND contype = 'p';"
            );
            rs.next();
            String primaryKey = rs.getString("conname");
            assertEquals("news_pkey", primaryKey, "Primary key constraint is missing or incorrect");
        }
        ...
        try (Statement stmt = connection.createStatement()) {
            ResultSet rs = stmt.executeQuery(
                    "SELECT conname " +
                            "FROM pg_constraint " +
                            "WHERE conrelid = 'public.news'::regclass AND contype = 'c';"
            );
            ...
            while (rs.next()) {
                ...
                if (checkConstraint.equals("news_status_check")) {
        ...
        try (Statement stmt = connection.createStatement()) {
            ResultSet rs = stmt.executeQuery(
                    "SELECT conname " +
                            "FROM pg_constraint " +
                            "WHERE conrelid = 'public.news'::regclass AND contype = 'f';"
            );
            ...
            while (rs.next()) {
                ...
                if (foreignKey.equals("news_author_id_foreign")) {
    ...
    @Test
    public void testNewsDataOperations() throws SQLException {
        // Вставка данных
        try (PreparedStatement pstmt = connection.prepareStatement(
                "INSERT INTO public.news (title, description, content, cover_uri, status, views, likes, reposts, created_at, updated_at, author_id) " +
                        "VALUES (?, ?::jsonb, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, ?);"
        )) {
            pstmt.setString(1, "Test News");
            ...
            assertEquals(1, rowsInserted, "Failed to insert data into news");
        }

        // Обновление данных
        try (PreparedStatement pstmt = connection.prepareStatement(
                "UPDATE public.news SET title = ? WHERE author_id = ?;"
        )) {
            pstmt.setString(1, "Updated News");
            pstmt.setInt(2, 1);
            int rowsUpdated = pstmt.executeUpdate();
            assertEquals(1, rowsUpdated, "Failed to update data in news");
        }

        // Проверка обновления данных
        try (PreparedStatement pstmt = connection.prepareStatement(
                "SELECT title FROM public.news WHERE author_id = ?;"
        )) {
            pstmt.setInt(1, 1);
            ResultSet rs = pstmt.executeQuery();
            rs.next();
            String updatedTitle = rs.getString("title");
            assertEquals("Updated News", updatedTitle, "Data update verification failed");
        }
        ...
        try (Statement stmt = connection.createStatement()) {
            stmt.executeUpdate("DELETE FROM public.user_login_data WHERE id = 1;");
            ResultSet rs = stmt.executeQuery(
                    "SELECT COUNT(*) FROM public.news WHERE author_id = 1;"
            );

            rs.next();
            int count = rs.getInt(1);
            assertEquals(0, count, "Failed to cascade delete data from news");
        }
    }
}
